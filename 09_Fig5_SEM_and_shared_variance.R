...
# ----------------------------------------------------------------------
ROOT <- getwd()
OUTDIR <- file.path(ROOT, "Fig5_SEM_output")
...
fit_models <- function(with_bacteria_to_root = FALSE) {
  models <- list(
    ...
    Yield = lm(
      Yield_s ~ RootActivity_s + M1_s + Year_c + Year2_c,
      data = dat
    )
  )
  models
...
basis_formulas <- function(with_branch = FALSE) {
  if (!with_branch) {
    return(list(
      ...
      Mowing_Yield =
        Yield_s ~ Mowing + RootActivity_s + M1_s + Year_c + Year2_c,
      Turnover_Metabolism =
      ...
      Turnover_Root =
        RootActivity_s ~ Turnover_s + MetabolicModule_s +
          Year_c + Year2_c,
      Turnover_Yield =
        Yield_s ~ Turnover_s + RootActivity_s + M1_s +
          Year_c + Year2_c,
      Bacteria_Root =
        RootActivity_s ~ BacterialBalance_s + MetabolicModule_s +
          Year_c + Year2_c,
      Bacteria_Yield =
        Yield_s ~ BacterialBalance_s + RootActivity_s + M1_s +
          Year_c + Year2_c,
      M1_Root =
        RootActivity_s ~ M1_s + MetabolicModule_s +
          Year_c + Year2_c,
      Metabolism_Yield =
        Yield_s ~ MetabolicModule_s + RootActivity_s + M1_s +
          Year_c + Year2_c
    ))
  }
  list(
...
ggsave(
  file.path(OUTDIR, "Fig5A_ModelA_without_RT_bacteria_to_root_activity.png"),
  pA,
  width = 11,
  height = 7,
  dpi = 600,
...
ggsave(
  file.path(OUTDIR, "Fig5A_ModelA_without_RT_bacteria_to_root_activity.svg"),
  pA,
  width = 11,
  height = 7,
  bg = "white"
...
ggsave(
  file.path(OUTDIR, "Fig5B_ModelB_with_RT_bacteria_to_root_activity.png"),
  pB,
  width = 11,
  height = 7,
  dpi = 600,
...
ggsave(
  file.path(OUTDIR, "Fig5B_ModelB_with_RT_bacteria_to_root_activity.svg"),
  pB,
  width = 11,
  height = 7,
  bg = "white"
...
# ----------------------------------------------------------------------
# 12. Main shared explanatory variance
# RT bacterial state + root metabolic module -> root activity
# after removing Year and Year^2.
# Commonality decomposition
# unique bacteria = R2(B+M) - R2(M)
# unique metabolism = R2(B+M) - R2(B)
# shared = R2(B) + R2(M) - R2(B+M)
# ----------------------------------------------------------------------
residualize <- function(var) {
  residuals(
    lm(
      reformulate(
        c("Year_c","Year2_c"),
...
vp_root <- tibble(
  RootActivity = residualize("RootActivity_s"),
  Bacteria = residualize("BacterialBalance_s"),
  Metabolism = residualize("MetabolicModule_s")
)
R2_B <- summary(lm(RootActivity ~ Bacteria, data = vp_root))$r.squared
R2_M <- summary(lm(RootActivity ~ Metabolism, data = vp_root))$r.squared
R2_BM <- summary(
  lm(RootActivity ~ Bacteria + Metabolism, data = vp_root)
)$r.squared
Unique_Bacteria <- R2_BM - R2_M
Unique_Metabolism <- R2_BM - R2_B
Shared_BM <- R2_B + R2_M - R2_BM
Unexplained <- 1 - R2_BM
vp_root_table <- tibble(
  Component = c(
    "Unique RT bacterial state",
    "Shared RT bacteria + metabolism",
    "Unique root metabolic module",
    "Unexplained"
  ),
  Fraction = c(
    Unique_Bacteria,
...
write_csv(
  vp_root_table,
  file.path(OUTDIR, "09_shared_variance_root_activity.csv")
)
vp_root_plot <- vp_root_table %>%
  mutate(
    Component = factor(
      Component,
      levels = Component
...
p_shared_root <- ggplot(
  ...
  coord_flip() +
  labs(
    title = "Shared explanatory variance of RT bacterial state and root metabolic module",
    subtitle = paste0(
      "Adjusted for stand age and stand age²; combined partial R² = ",
      sprintf("%.3f", R2_BM),
      "; shared = ",
      sprintf("%.1f", 100 * Shared_BM / R2_BM),
      "% of explained variance"
    ),
    x = NULL,
    y = "Variance in root activity (%)",
    fill = NULL
  ) +
...
ggsave(
  file.path(OUTDIR, "Fig5C_shared_variance_root_activity.png"),
  p_shared_root,
  width = 10,
  height = 4.6,
  dpi = 600,
...
ggsave(
  file.path(OUTDIR, "Fig5C_shared_variance_root_activity.svg"),
  p_shared_root,
  width = 10,
  height = 4.6,
  bg = "white"
...
# ----------------------------------------------------------------------
# 13. Figure 5D shared variance
# M1 + RT bacterial state -> root metabolic module
# This panel emphasizes M1's central position while showing that
# bacteria and M1 carry a strongly shared metabolic signal.
# ----------------------------------------------------------------------
vp_met <- tibble(
  Metabolism = residualize("MetabolicModule_s"),
  M1 = residualize("M1_s"),
...
R2_M1 <- summary(lm(Metabolism ~ M1, data = vp_met))$r.squared
R2_B2 <- summary(lm(Metabolism ~ Bacteria, data = vp_met))$r.squared
R2_M1B <- summary(
  lm(Metabolism ~ M1 + Bacteria, data = vp_met)
)$r.squared
Unique_M1 <- R2_M1B - R2_B2
Unique_Bacteria2 <- R2_M1B - R2_M1
Shared_M1B <- R2_M1 + R2_B2 - R2_M1B
Unexplained2 <- 1 - R2_M1B
vp_met_table <- tibble(
  Component = c(
    "Unique M1",
    "Shared M1 + RT bacteria",
    "Unique RT bacterial state",
    "Unexplained"
  ),
  Fraction = c(
...
write_csv(
  vp_met_table,
  file.path(OUTDIR, "10_shared_variance_root_metabolic_module.csv")
)
p_shared_met <- ggplot(
  vp_met_table,
  aes(
    x = "Root metabolic module",
    y = Percent,
    fill = Component
  )
) +
  ...
  coord_flip() +
  labs(
    title = "Shared explanatory variance of M1 and RT bacterial state",
    subtitle = paste0(
      "Adjusted for stand age and stand age²; combined partial R² = ",
      sprintf("%.3f", R2_M1B),
      "; shared = ",
      sprintf("%.1f", 100 * Shared_M1B / R2_M1B),
      "% of explained variance"
    ),
    x = NULL,
    y = "Variance in root metabolic module (%)",
    fill = NULL
  ) +
  theme_classic(base_size = 11) +
  theme(
...
ggsave(
  file.path(OUTDIR, "Fig5D_shared_variance_root_metabolic_module.png"),
  p_shared_met,
  width = 10,
  height = 4.6,
  dpi = 600,
...
ggsave(
  file.path(OUTDIR, "Fig5D_shared_variance_root_metabolic_module.svg"),
  p_shared_met,
  width = 10,
  height = 4.6,
  bg = "white"
...
cat("============================================================\n")
cat("\nModel A, metabolism -> root activity:\n")
print(compA)
cat("\nModel B, metabolism -> root activity:\n")
print(compB_met)
cat("\nModel B, RT bacteria -> root activity:\n")
print(compB_bac)
cat("\nGlobal fit table:\n")
print(fit_table)
cat("\nRoot-activity shared variance:\n")
print(vp_root_table)
cat(
  "\nShared RT bacteria + metabolism = ",
  round(100 * Shared_BM, 1),
  "% of total root-activity variance, and ",
  round(100 * Shared_BM / R2_BM, 1),
  "% of explained variance.\n",
  sep = ""
)
cat("\nOutput directory:\n", OUTDIR, "\n")
